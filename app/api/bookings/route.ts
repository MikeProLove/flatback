...
export async function POST(req: Request) {
  try {
    ...
    const monthly_price =
      typeof body.monthlyPrice === 'number' ? body.monthlyPrice : (L.data.price as number | null) ?? null;
    const deposit =
      typeof body.deposit === 'number' ? body.deposit : (L.data.deposit as number | null) ?? null;
    ...
    async function insertInto(table: 'bookings' | 'bookings_base') {
      return sb
        .from(table)
        .insert({
    ...
    // необязательный шаг: обеспечим чат между владельцем объявления и заявителем
    ...
    return NextResponse.json({ id: ins.data.id, chatId, chatPath: chatId ? `/chat/${chatId}` : null });
  } catch (e: any) {
