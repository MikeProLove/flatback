...
// динамические импорты (типизацию упрощаем до any)
const Map = NextDynamic(() => import('react-leaflet').then(m => m.MapContainer), { ssr: false }) as any;
const TileLayer = NextDynamic(() => import('react-leaflet').then(m => m.TileLayer), { ssr: false }) as any;
const Marker = NextDynamic(() => import('react-leaflet').then(m => m.Marker), { ssr: false }) as any;
const Popup = NextDynamic(() => import('react-leaflet').then(m => m.Popup), { ssr: false }) as any;
const Circle = NextDynamic(() => import('react-leaflet').then(m => m.Circle), { ssr: false }) as any;
    ...
    <div className="h-[calc(100vh-70px)] relative">
      ...
      <div className="absolute z-[1000] right-3 top-3 pointer-events-none" style={{ width: 260 }}>
        <div className="rounded-xl border bg-white/95 shadow p-3 space-y-2 pointer-events-auto">
          ...
          <input type="range" min={1} max={20} value={radiusKm} onChange={(e) => setRadiusKm(Number(e.target.value))} className="w-full" />
      ...
      <Map center={center} zoom={12} whenCreated={(m: any) => (mapRef.current = m)} style={{ height: '100%', width: '100%' }}>
        <TileLayer attribution="&copy; OpenStreetMap" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
        <Circle center={center} radius={radiusKm * 1000} />
        {rows.filter(r => r.lat != null && r.lng != null).map(r => (
