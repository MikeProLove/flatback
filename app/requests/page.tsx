...
async function getMy(userId: string) {
  ...
  const { data } = await sb
    ...
    .select('id,status,start_date,end_date,monthly_price,deposit,created_at,listing_id,owner_id, listings:listing_id(title,city)')
    .eq('tenant_id', userId)
...
async function cancel(id: string) {
  // server component — не вызываем; отмена через client/actions, если понадобится
}
            ...
            <div key={r.id} className="rounded-2xl border overflow-hidden">
              <div className="grid grid-cols-[160px_1fr] gap-0">
                ...
                <div className="p-4 space-y-1">
                  ...
                  {r.status === 'pending' ? (
                    <div className="pt-2">
                      <form action={`/api/bookings/${r.id}`} method="POST" onSubmit={(e) => e.preventDefault()}>
                        <button
                          onClick={async () => {
